...
object BulkCopyUtils extends Logging {
        ...
        val colMetaData = {
            if(checkSchema) {
                ...
                matchSchemas(
                    ...
                    options.url,
                    isCaseSensitive,
                    options.schemaCheckEnabled,
                    options.preferTimestampNTZ)
            } else {
    ...
    */
    private[spark] def matchSchemas(
            ...
            preferTimestampNTZ: Boolean): Array[ColumnMetadata]= {
        val dfColCaseMap = (df.schema.fieldNames.map(item => item.toLowerCase)
          zip df.schema.fieldNames.toList).toMap
        val dfCols = df.schema

        val tableCols = getSchema(conn, rs, JdbcDialects.get(url), false, preferTimestampNTZ)
